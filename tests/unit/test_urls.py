...
def test_set_components():
    ...
    # Replace single item
    aurl_new = boxfish.utils.urls.set_components(aurl, scheme=scheme)
    aurl_expected = 'ftp://user:pwd@NetLoc.com:80/p1;para/p2;para?query=arg#frag'
    assert aurl_new == aurl_expected

    aurl_new = boxfish.utils.urls.set_components(aurl, netloc=netloc)
    aurl_expected = 'http://newloc.com/p1;para/p2;para?query=arg#frag'
    assert aurl_new == aurl_expected

    # Replace two items
    aurl_new = boxfish.utils.urls.set_components(aurl, scheme=scheme, netloc=netloc)
    aurl_expected = 'ftp://newloc.com/p1;para/p2;para?query=arg#frag'
    assert aurl_new == aurl_expected

    # Replace query from dict
    aurl_new = boxfish.utils.urls.set_components(aurl, query=query)
    aurl_expected = 'http://user:pwd@NetLoc.com:80/p1;para/p2;para?query1=arg1&query2=arg2#frag'
    assert aurl_new == aurl_expected

    # Replace path
    aurl_new = boxfish.utils.urls.set_components(aurl, path=path)
    aurl_expected = 'http://user:pwd@NetLoc.com:80/p11/p22?query=arg#frag'
    assert aurl_new == aurl_expected

    # Replace path_no_backlash
    aurl_new = boxfish.utils.urls.set_components(aurl, path=path_no_backlash)
    aurl_expected = 'http://user:pwd@NetLoc.com:80/p11/p22?query=arg#frag'
    assert aurl_new == aurl_expected

    # Replace items from dict
    aurl_new = boxfish.utils.urls.set_components(aurl, **cdict)
    aurl_expected = 'ftp://newloc.com/p1;para/p2;para?query=arg#frag'
    assert aurl_new == aurl_expected

    # Replace items from dict with unknown keys
    aurl_new = boxfish.utils.urls.set_components(aurl, **ddict)
    aurl_expected = 'ftp://newloc.com/p1;para/p2;para?query=arg#frag'
    assert aurl_new == aurl_expected
...
def test_get_components():
    ...
    adict = boxfish.utils.urls.get_components(aurl)
...
def test_replace_subpath():
    ...
    aurl = boxfish.utils.urls.replace_subpath(url_original, subpath, level)
...
def test_replace_subquery():
    ...
    aurl = boxfish.utils.urls.replace_subquery(url_original, subquery)
    assert aurl_expected == aurl
...
def test_create_url_list():
    ...
    aurl_list = boxfish.utils.urls.create_url_list(url_original, query=query)
    ...
    aurl_list = boxfish.utils.urls.create_url_list(url_original, path=path)
    assert aurl_list == aurl_list_expected
    ...
    aurl_list = boxfish.utils.urls.create_url_list(url_original, query=query, path=path)
    assert aurl_list == aurl_list_expected
...
def test_valid_http():
    ...
    assert boxfish.utils.urls.valid_http(url_http)
    assert boxfish.utils.urls.valid_http(url_https)
    assert not boxfish.utils.urls.valid_http(url_no_http)
    assert not boxfish.utils.urls.valid_http(url_no_scheme)
...
def test__dict_to_path():
    ...
    path = boxfish.utils.urls._dict_to_path(adict)
...
def test__path_to_dict():
    ...
    adict = boxfish.utils.urls._path_to_dict(path)
    assert adict_expected == adict
...
def test__dict_to_query():
    ...
    query = boxfish.utils.urls._dict_to_query(adict)
...
def test__query_to_dict():
    ...
    adict = boxfish.utils.urls._query_to_dict(query)
    assert adict_expected == adict
